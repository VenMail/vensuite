...
export function equalNodeType(nodeType, node) {
...
export function replaceNodeAtPos(position, content) {
  return (tr) => {
...
export function canReplace($pos, content) {
...
// `position` should point at the position immediately before the node.
export function removeNodeAtPos(position) {
  return (tr) => {
    const node = tr.doc.nodeAt(position)
...
// }
// ```
export function canInsert($pos, content) {
...
export function checkInvalidMovements(originIndex, targetIndex, targets, type) {
  ...
  if (direction === 1) {
    ...
    if (targets.slice(1).includes(targetIndex)) {
      throw new Error(errorMessage)
    }
  }
