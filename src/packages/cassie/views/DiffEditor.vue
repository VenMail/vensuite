...
import { footerlist, headerlist, newContent, pageOldContent } from './content'
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      newEditor.value = new Editor(Object.assign({}, option, { content: newContent, extensions }))
      oldEditor.value = new Editor(Object.assign({}, option, { content: pageOldContent, extensions }))
      const startState = EditorState.create({ doc: oldD })
      let domSerializer: DOMSerializer
      diff.value = new Editor(Object.assign({}, option, { content: tr.doc.toJSON(), extensions }))
...
<template>
  <div class="flex place-content-center bg-gray-200">
    <div class="my-2">
      ...
      <EditorContent v-show="active == 1" :editor="diff" />
      <EditorContent v-show="active == 2" :editor="newEditor" />
      <EditorContent v-show="active == 3" :editor="oldEditor" />
    </div>
  </div>
</template>
