      ...
      <div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
        ...
        <div v-if="ownerMember" class="mb-3">
          <div class="flex items-center gap-3 py-2">
            ...
            <span class="text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">{{$t('Commons.text.other')}}</span>
          </div>
        </div>
        ...
        <div v-if="otherMembers.length > 0" class="space-y-2">
          <div v-for="member in otherMembers" :key="member.email" class="flex items-center justify-between py-2">
            ...
            <div class="flex items-center gap-2 ml-4">
              ...
              >
                <option value="view">{{$t('Commons.text.view')}}</option>
                <option value="comment">{{$t('Commons.text.comment')}}</option>
                <option value="edit">{{$t('Commons.text.edit')}}</option>
              </select>
              <button
    ...
    <div v-else-if="currentPage === 'invite'" class="space-y-6">
      ...
      <div>
        ...
        <div class="flex gap-2">
          ...
          />
          <Button variant="outline" @click="addInvitee" :disabled="!newInviteEmail.trim()">{{$t('Commons.button.add')}}</Button>
        </div>
      </div>
