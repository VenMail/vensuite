...
async function init(data: IWorkbookData = DEFAULT_WORKBOOK_DATA) {
  try {
    ...
    // If there's an existing instance, dispose it first to avoid conflicts with new container
    if (globalUniverInstance) {
      console.warn('Disposing previous Univer instance to attach to new container');
      try {
        globalUniverInstance.dispose();
      } catch (e) {
        console.warn('Failed to dispose previous Univer instance:', e);
      }
    ...
    globalUniverInstance = univer.univer;
