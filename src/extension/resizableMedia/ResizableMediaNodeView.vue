...
function onHorizontalResize(directionOfMouseMove: 'right' | 'left', diff: number) {
  if (!resizableImg.value) {
...
function onHorizontalMouseMove(e: MouseEvent) {
...
function onVerticalMouseMove(e: MouseEvent) {
...
<template>
  <NodeViewWrapper as="article" class="media-node-view flex pos-relative not-prose" :class="[`${(isFloat && `f-${props.node.attrs.dataFloat}`) || ''}`, `${(isAlign && `align-${props.node.attrs.dataAlign}`) || ''}`]">
