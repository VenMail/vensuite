      ...
      >
        <div class="flex items-center justify-between px-3 sm:px-4 md:px-6 py-2 sm:py-3 border-b border-gray-200/70 dark:border-gray-800/70">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-300">{{$t('Views.Sheets.text.pick_a_template_or_start_blank')}}</p>
          </div>
...
import { spreadsheetTemplateDefinitions } from "@/constants/sheetTemplates";
import {
  useFileExplorer,
...
const spreadsheetTemplates: SpreadsheetTemplate[] = spreadsheetTemplateDefinitions.map((template) => ({
  name: template.name,
  slug: template.slug,
...
const topBarActions = computed(() => {
  const actions = [
    ...
    },
    {
      ...
      icon: Upload,
      component: Button,
      props: iconActionProps("Upload spreadsheets"),
      ...
      actions.push(
        {
          ...
          icon: FolderOpen,
          component: Button,
          props: iconActionProps("Open selected spreadsheet"),
        ...
        },
        {
          ...
          icon: Edit,
          component: Button,
          props: iconActionProps("Rename selected spreadsheet"),
    ...
    actions.push(
      {
        ...
        icon: Download,
        component: Button,
        props: iconActionProps("Download selected spreadsheets"),
      ...
      },
      {
        ...
        icon: Trash2,
        component: Button,
        props: iconActionProps("Move selected spreadsheets to trash"),
...
function handleBulkDownload() {
  const files = Array.from(selectedFiles.value)
    .map(id => sortedSpreadsheets.value.find(f => f.id === id))
