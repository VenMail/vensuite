<template>
  <template v-if="isAuthenticated">
    <AppMenu />
    <div
      class="bg-gray-100 p-4 rounded-lg backdrop-blur backdrop-filter bg-opacity-60"
    >
      <form>
        <div class="relative">
          ...
          <Input
            v-model="searchValue"
            placeholder="Search..."
            class="pl-7 md:w-[100px] lg:w-[300px]"
          />
        </div>
      </form>
    </div>
    ...
    <div class="flex h-full p-4">
      ...
      <div class="h-full w-full flex flex-col items-center justify-center">
        <div
          v-if="files.length === 0"
          class="flex flex-1 py-8 my-auto justify-center shadow-sm"
        >
  ...
  <template v-else>
    <div class="h-full w-full flex flex-col items-center justify-center">
      <div class="flex flex-1 py-8 my-auto justify-center shadow-sm">
        <div class="flex flex-col items-center gap-1 text-center">
          <Button class="mt-4" @click="loginWithVenmail">
            Login with Venmail
          </Button>
          <p class="text-sm text-muted-foreground">
