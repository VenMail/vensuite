...
const slideRooms = new Map();            // slideId -> Set<WebSocket>
const slideChatMessages = new Map();     // slideId -> Array<SlideChatMessage>
...
function createWebSocketApp() {
  ...
  app.ws("/slides/*", {
    ...
    upgrade: (res, req, context) => {
      const slideId = req.getQuery('slideId');
      const userId = req.getQuery("userId");
      const userName = req.getQuery("userName");
      const channel = req.getQuery("channel") || "default";

      if (!slideId || !userId || !userName) {
        return res.cork(() => res.writeStatus("400 Bad Request").end("Missing slideId, userId, or userName"));
      }
    ...
    open: (ws) => {
      ...
      // Add to slide room connections
      if (ws.slideId) {
        if (!slideRooms.has(ws.slideId)) {
          slideRooms.set(ws.slideId, new Set());
        }
        slideRooms.get(ws.slideId).add(ws);

        // Subscribe the client to the slide topic
        ws.subscribe(`slides:${ws.slideId}`);

        logger.info(`Client joined slide room and subscribed to topic`, {
          slideId: ws.slideId,
          userId: ws.userId,
        ...
        // Send chat history for non-CRDT channels
        if (ws.channel !== 'crdt') {
          sendSlideChatHistory(ws);
        }

        // Notify other users about the new user joining (except for CRDT-only connections)
        if (ws.channel !== 'crdt') {
          const connections = slideRooms.get(ws.slideId) || new Set();
          const users = Array.from(connections)
            .filter(s => s.channel !== 'crdt' && s !== ws)
    ...
    message: (ws, message, isBinary) => {
      // Binary frames: treat as CRDT/Yjs updates and relay within the same slide room
      if (isBinary) {
        if (ws.slideId && slideRooms.has(ws.slideId)) {
          try {
            const peers = slideRooms.get(ws.slideId);
            ...
            logger.error("Error relaying binary update for slides", { slideId: ws.slideId, error: e.message });
          }
        } else {
          logger.debug("Received binary message outside a slide room; ignoring");
        }
        return;
      }
      ...
      try {
        ...
        // Handle different message types
        if (data.type === 'chat_message') {
          handleSlideChatMessage(ws, data);
        } else if (data.type === 'get_chat_history') {
          sendSlideChatHistory(ws);
        } else if (data.type === 'slide_operation' || data.type === 'cursor' || data.type === 'title') {
          handleSlideOperation(ws, data);
        } else {
          logger.warn('Unknown slide message type', { type: data.type, slideId: ws.slideId });
        }
      } catch (error) {
        logger.error('Error processing slide message', { slideId: ws.slideId, error: error.message });
      }
    },

    close: (ws, code, message) => {
      const slideId = ws.slideId;
      logger.info(`Client disconnected from slide room: ${slideId}`, { code, userId: ws.userId });

      // Unsubscribe from the slide topic
      ws.unsubscribe(`slides:${slideId}`);

      // Remove from slide room connections
      if (slideRooms.has(slideId)) {
        slideRooms.get(slideId).delete(ws);

        // Clean up empty slide rooms
        if (slideRooms.get(slideId).size === 0) {
          slideRooms.delete(slideId);
        }

        // Notify other users about the user leaving (except for CRDT-only connections)
        if (ws.channel !== 'crdt') {
          broadcastToSlide(
            slideId,
            JSON.stringify({
  ...
  app.ws("/*", {
    ...
    upgrade: (res, req, context) => {
      ...
      if (sheetId) {
        ...
        handleSlideUpgrade(res, req, context, slideId, abort);
      } else if (token) {
        try {
    ...
    message: (ws, message, isBinary) => {
      ...
      // Binary frames: treat as CRDT/Yjs updates and relay within the same room
      if (isBinary) {
        if (ws.sheetId && sheetSockets.has(ws.sheetId)) {
        ...
        } else if (ws.slideId && slideRooms.has(ws.slideId)) {
          try {
            const peers = slideRooms.get(ws.slideId);
            peers.forEach((client) => {
              if (client !== ws) {
          ...
          logger.debug("Received binary message outside a room; ignoring");
        }
        return;
      }
...
function handleSlideUpgrade(res, req, context, slideId, abort) {
  const userId = req.getQuery("userId");
  const userName = req.getQuery("userName");
  const channel = req.getQuery("channel") || "default";
  const secWebSocketKey = req.getHeader("sec-websocket-key");
  ...
  if (abort.done) return;
  res.upgrade(
    { slideId, userId, userName, channel },
    secWebSocketKey,
    secWebSocketProtocol,
    secWebSocketExtensions,
    context
...
function handleTokenUpgrade(res, req, context, token, abort) {
  ...
  cache.get(token, (err, serializedData) => {
    ...
    if (err || !tokenData || !tokenData.user_id || new Date() > new Date(tokenData.valid_until)) {
      return res.cork(() => res.writeStatus("401 Unauthorized").end("Invalid token data"));
    }

    const orgChatEnabled = tokenData.org_chat_enabled !== false;
    const hasAccounts = Array.isArray(tokenData.accounts) && tokenData.accounts.length > 0;

    if (!hasAccounts && !orgChatEnabled) {
      return res.cork(() => res.writeStatus("403 Forbidden").end("No valid accounts"));
    }

    const finalizeUpgrade = (accounts) => {
      activeTokens.set(token, {
        userId: tokenData.user_id,
        accounts,
        ws: null,
        org_chat_enabled: orgChatEnabled
      });
...
function handleSlideOpen(ws) {
  ...
  if (ws.channel !== 'crdt') {
    // Send initialization data (exclude CRDT sockets from users list)
    getSlideChatMessages(ws.slideId)
      .then(messages => {
        const connections = slideRooms.get(ws.slideId) || new Set();
        const users = Array.from(connections)
          .filter(s => s.channel !== 'crdt' && s !== ws)
      ...
      .catch(error => {
        logger.error("Failed to load cached slide messages", {
          slideId: ws.slideId,
          error: error.message
        });
      });
  ...
  logger.info("Slide connection opened", {
    slideId: ws.slideId,
    userId: ws.userId,
    userName: ws.userName
  });
...
function handleSlideClose(ws) {
  if (slideRooms.has(ws.slideId)) {
    const connections = slideRooms.get(ws.slideId);
    connections.delete(ws);

    if (connections.size === 0) {
      slideRooms.delete(ws.slideId);
    }
  ...
  logger.info("Slide connection closed", {
    slideId: ws.slideId,
    userId: ws.userId
  });
}
...
function handleSlideMessage(ws, rawMessage) {
  try {
    const message = JSON.parse(rawMessage);
    logger.debug("Received slide message", {
      slideId: ws.slideId,
      userId: ws.userId,
      type: message.type
    });
    ...
    switch (message.type) {
      case "join":
        // Join is handled during connection setup
        break;
      case "leave":
        // Leave is handled during connection close
        break;
      case "chat":
        handleSlideChatMessage(ws, message);
        break;
      case "cursor":
      case "slide_operation":
      case "title":
        handleSlideOperation(ws, message);
        break;
      default:
        logger.warn("Unknown slide message type", { type: message.type });
    }
  } catch (error) {
    logger.error("Slide message processing failed", {
      slideId: ws.slideId,
      userId: ws.userId,
      error: error.message
    });
...
async function handleSlideChatMessage(ws, message) {
  const chatMessage = {
    id: `${ws.userId}-${Date.now()}`,
    type: "chat",
    content: message.content,
  ...
  await storeSlideChatMessage(ws.slideId, chatMessage);
  broadcastToSlide(ws.slideId, JSON.stringify(chatMessage));
}

function handleSlideOperation(ws, message) {
  const operation = {
    ...message,
    user: { id: ws.userId, name: ws.userName },
    timestamp: Date.now(),
  ...
  broadcastToSlide(ws.slideId, JSON.stringify(operation), ws);
}

// Slide Utilities
async function storeSlideChatMessage(slideId, message) {
  try {
    const messages = await getSlideChatMessages(slideId);
    messages.push(message);
    ...
    logger.error("Failed to store slide chat message", {
      slideId,
      error: error.message
    });
  }
...
function getSlideChatMessages(slideId) {
  return new Promise((resolve, reject) => {
    if (slideChatMessages.has(slideId)) {
      return resolve(slideChatMessages.get(slideId));
    }
    cache.get(`slide_messages_${slideId}`, (err, data) => {
      if (err) return reject(err);
      try {
        const messages = data ? JSON.parse(data) : [];
        slideChatMessages.set(slideId, messages);
        resolve(messages);
      } catch (e) {
        logger.error("Corrupted slide chat cache data", { slideId });
        slideChatMessages.set(slideId, []);
        resolve([]);
      }
    });
  });
...
function broadcastToSlide(slideId, message, excludeWs = null) {
  const connections = slideRooms.get(slideId);
  if (connections) {
    connections.forEach(client => {
      if (client !== excludeWs) {
        safeSend(client, message);
...
async function sendSlideChatHistory(ws) {
  if (!ws.slideId) {
    logger.error('Cannot send slide chat history for unknown slide', { ws });
    return;
  }

  try {
    // Get the chat history
    const messages = await getSlideChatMessages(ws.slideId);
    ...
    logger.debug('Sent slide chat history to client', {
      slideId: ws.slideId,
      messageCount: recentMessages.length
    });
  } catch (error) {
    logger.error('Error sending slide chat history', {
      slideId: ws.slideId,
      error: error.message
    });
  }
